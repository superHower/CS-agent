...
export default function FaqManage() {
  ...
  useEffect(() => {
    fetch(`${apiUrl}/shops`)
      ...
      .then((data: { shop_id: string; name: string }[]) =>
        setShops(Array.isArray(data) ? data.map((s) => ({ id: s.shop_id, name: s.name })) : [])
      )
      .catch(() => notify("加载店铺列表失败", { type: "error" }));
  }, [notify]);
  ...
  useEffect(() => { loadFaqs(); }, [shopId, category]); // eslint-disable-line
  ...
  const openCreate = () => {
    setEditingId(null);
    setForm({ ...EMPTY_FORM, aliases: [{ question: "", is_primary: true }] });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const body = { shop_id: shopId, answer: form.answer, category: form.category, priority: form.priority, enabled: form.enabled, aliases: form.aliases };
    try {
      const res = editingId
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.value = "";
    try {
      const res = await fetch(`${apiUrl}/faqs/import?shop_id=${shopId}`, { method: "POST", body: fd });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v: string) => (
        <Tooltip title={v}>
          <Text ellipsis style={{ maxWidth: 280, display: "block" }}>{v}</Text>
        </Tooltip>
      ),
    },
    {
      ...
      <Card style={{ marginBottom: 16 }}>
        <Space wrap>
          ...
          <Button
            ...
            disabled={!shopId || importing}
            loading={importing}
          >
            CSV 导入
          </Button>
          <input ref={fileInputRef} type="file" accept=".csv" hidden onChange={handleImport} />
          <Button type="primary" icon={<PlusOutlined />} onClick={openCreate} disabled={!shopId}>
            新增 FAQ
          </Button>
        </Space>
      </Card>
