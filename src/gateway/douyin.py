...
通过抖店 Webhook 推送接收买家消息，调用抖店开放 API 发送消息。
"""
...
_DOUYIN_API_URL = "https://openapi-fxg.jinritemai.com"
_DEDUP_KEY_PREFIX = "msg_dedup"


def _douyin_sign(method: str, params: dict[str, str], app_secret: str) -> str:
    """计算抖店 API 签名（HMAC-SHA256 方式）。

    签名字符串 = method + sorted(param_key+param_value) + app_secret
    """
    sorted_str = "".join(f"{k}{v}" for k, v in sorted(params.items()))
    sign_str = method + sorted_str + app_secret
    return hmac.new(app_secret.encode(), sign_str.encode(), hashlib.sha256).hexdigest()


class DouyinGateway(BaseGateway):
    """抖店开放平台网关。"""
    ...
    async def _is_duplicate(self, shop_id: str, message_id: str) -> bool:
        ...
        try:
            ...
            logger.warning("抖店消息去重 Redis 失败，跳过去重: %s", exc)
            return False
    ...
    def _parse_message(self, shop_config: ShopConfig, raw: dict) -> StandardMessage | None:
        ...
        try:
            # 抖店消息字段映射
            buyer_id = str(raw.get("open_id") or raw.get("buyer_id") or raw.get("from_user_id", ""))
            content = str(raw.get("content") or raw.get("msg") or raw.get("text", ""))
            message_id = str(raw.get("message_id") or raw.get("msg_id") or raw.get("id", ""))
            ts_ms = raw.get("timestamp") or raw.get("create_time") or int(time.time() * 1000)

            if not buyer_id or not content or not message_id:
                logger.warning("抖店消息字段缺失 shop=%s", shop_config.shop_id)
                return None
            ...
            logger.error("抖店消息解析失败 shop=%s: %s", shop_config.shop_id, exc)
            return None
    ...
    def _build_app(self, shop_configs: list[ShopConfig]):
        ...
        async def handle_webhook(request: web.Request) -> web.Response:
            ...
            for msg_raw in messages:
                ...
                await queue.put(msg)
                logger.info("抖店消息入队 shop=%s buyer=%s", shop_id, msg.buyer_id)
    ...
    async def send(
        ...
        params["sign"] = _douyin_sign(method, params, shop_config.api_secret)

        url = f"{_DOUYIN_API_URL}/{method.replace('.', '/')}"

        try:
            timeout = aiohttp.ClientTimeout(total=3.0)
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(url, json={**params, "method": method}) as resp:
                    resp_json = await resp.json(content_type=None)

            if resp_json.get("err_no", 0) != 0:
                logger.error(
                    "抖店发送失败 shop=%s buyer=%s resp=%s",
                    shop_config.shop_id,
                    buyer_id,
                    resp_json,
                )
            ...
            logger.info("抖店消息发送成功 shop=%s buyer=%s", shop_config.shop_id, buyer_id)
            return True

        except aiohttp.ClientError as exc:
            logger.error("抖店发送网络异常 shop=%s: %s", shop_config.shop_id, exc)
            return False
        except Exception as exc:
            logger.error("抖店发送未知异常 shop=%s: %s", shop_config.shop_id, exc, exc_info=True)
            return False
