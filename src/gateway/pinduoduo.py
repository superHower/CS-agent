...
通过拼多多 Webhook 推送接收买家消息，调用拼多多开放 API 发送消息。
"""
...
_PDD_API_URL = "https://gw-api.pinduoduo.com/api/router"
_DEDUP_KEY_PREFIX = "msg_dedup"


def _pdd_sign(params: dict[str, str], secret: str) -> str:
    """计算拼多多 API 签名（MD5 方式）。

    排序所有参数 key+value 拼接后，首尾加 secret，取 MD5 大写。
    """
...
class PinduoduoGateway(BaseGateway):
    """拼多多开放平台网关。

    与千牛网关接口完全一致，仅内部消息格式适配不同。
    """
    ...
    async def _is_duplicate(self, shop_id: str, message_id: str) -> bool:
        ...
        try:
            ...
            logger.warning("拼多多消息去重 Redis 失败，跳过去重: %s", exc)
            return False
    ...
    def _parse_message(self, shop_config: ShopConfig, raw: dict) -> StandardMessage | None:
        ...
        try:
            # 拼多多消息字段映射
            buyer_id = str(raw.get("buyer_user_id") or raw.get("user_id") or raw.get("userId", ""))
            content = str(raw.get("msg_content") or raw.get("content") or raw.get("message", ""))
            message_id = str(raw.get("msg_id") or raw.get("message_id") or raw.get("msgId", ""))
            ts_ms = raw.get("timestamp") or raw.get("create_time") or int(time.time() * 1000)

            if not buyer_id or not content or not message_id:
                logger.warning("拼多多消息字段缺失 shop=%s", shop_config.shop_id)
                return None
            ...
            logger.error("拼多多消息解析失败 shop=%s: %s", shop_config.shop_id, exc)
            return None
    ...
    def _build_app(self, shop_configs: list[ShopConfig]):
        ...
        async def handle_webhook(request: web.Request) -> web.Response:
            ...
            for msg_raw in messages:
                ...
                await queue.put(msg)
                logger.info("拼多多消息入队 shop=%s buyer=%s", shop_id, msg.buyer_id)
    ...
    async def send(
        ...
        """通过拼多多开放 API 发送消息。"""
        params: dict[str, str] = {
            "type": "pdd.im.message.send",
            "client_id": shop_config.api_key,
            "timestamp": str(int(time.time())),
            "data_type": "JSON",
            "to_id": buyer_id,
            "msg_type": str(metadata.get("msg_type", 1)),
            "content": content,
        }
        params["sign"] = _pdd_sign(params, shop_config.api_secret)

        try:
            timeout = aiohttp.ClientTimeout(total=3.0)
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(_PDD_API_URL, data=params) as resp:
                    resp_json = await resp.json(content_type=None)

            if resp_json.get("error_response"):
                err = resp_json["error_response"]
                logger.error(
                    "拼多多发送失败 shop=%s buyer=%s error=%s",
                    shop_config.shop_id,
                    buyer_id,
                    err,
                )
            ...
            logger.info("拼多多消息发送成功 shop=%s buyer=%s", shop_config.shop_id, buyer_id)
            return True

        except aiohttp.ClientError as exc:
            logger.error("拼多多发送网络异常 shop=%s: %s", shop_config.shop_id, exc)
            return False
        except Exception as exc:
            logger.error("拼多多发送未知异常 shop=%s: %s", shop_config.shop_id, exc, exc_info=True)
            return False
